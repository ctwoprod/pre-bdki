...
public final class SqlUtil {
	 ...
	 * @author dbejar
	 */
	public static String createSingleWhereClause(String propertyName, Object value,
			QueryComparator comparator, Boolean... isLowercase) {
		...
		if (propertyName.contains(DOT) && !propertyName.contains(DOT_PROPERTY_ID)) {
			appendModel = "";
		} else {
			appendModel = MODEL_ALIAS;
		}
		if (value == null) {
			queryString.append(appendModel).append(propName).append(" IS NULL ");
		} else {
			...
			if (value instanceof String
					&& (SqlUtil.isLowercaseSetToYesOrNull(isLowercase) && !SqlUtil
							.isAnId(propertyName))) {
				queryString.append(" lower(str(").append(appendModel).append(propName)
						.append(")) ");
				queryString.append(comparator).append(QUERY_PARAMETER);
			} else {
				queryString.append(SPACE).append(appendModel).append(propName).append(SPACE);
				queryString.append(comparator).append(QUERY_PARAMETER);
			}
		}
	...
	public static String createSingleWhereClause(List<Object> parameters,
		...
		StringBuilder queryString = new StringBuilder();
		if (propertyName.contains(MODEL_ALIAS) && !propertyName.contains(DOT_PROPERTY_ID)) {
			model = "";
		} else {
			model = MODEL_ALIAS;
		}
		if (isAnId(propertyName)) {
			...
			if (value instanceof String) {
				queryString.append(" lower(str(").append(model).append(propertyName).append(")) ");
			} else {
				queryString.append(SPACE).append(model).append(propertyName).append(SPACE);
			}
		}
	 ...
	 *            query string.
	 */
	private static void addExpressionClause(PropCriteriaAndValue filter, List<Object> parameters,
	...
	private static void addInExpression(StringBuilder queryString, List<Object> parameters,
			Object propValue) {
	 ...
	 */
	@SuppressWarnings(UNCHECKED)
	private static void addBetweenExpression(StringBuilder queryString, List<Object> parameters,
			Object propValue) {
		queryString.append(" between ").append(QUERY_PARAMETER).append(" and ").append(
				QUERY_PARAMETER).append(SPACE);
	 ...
	 */
	@SuppressWarnings(UNCHECKED)
	public static List<T> doQuery(Query query, int... rowStartIdxAndCount) {
	...
	public static boolean isAnId(String propertyName) {
		return propertyName.endsWith(DOT_PROPERTY_ID) || propertyName.equalsIgnoreCase(PROPERTY_ID)
