...
import org.fernandez.roberto.bdki.common.PaginationResult;
...
import org.fernandez.roberto.bdki.common.exception.CoreExceptionText;
import org.fernandez.roberto.bdki.common.exception.CorePersistenceException;
import org.fernandez.roberto.bdki.dao.core.CoreDAO;
 ...
 * @param <T>
 *            the generic type
 * @see es.cimsa.framework.common.persistence.domain.knowment.ncsisserver.persistence.core.domain.CoreDomain
 * @see es.cimsa.framework.common.persistence.dao.CoreDao
 * @author dbejar
 */
public abstract class CoreDAOImpl<T> extends FindLayerDAOImpl<T> implements
		CoreDAO<T> {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = -4586718400993944857L;
	 ...
	 *            annotation
	 */
	public CoreDAOImpl(Class<T> clazz, SessionFactory sessionFactory) {
	...
	public T insert(T instance) {
		...
		try {
			//instance.setId((String) getHibernateTemplate().save(instance));
			getHibernateTemplate().save(instance);
			LOG.debug(I18N_INSERT + I18N_SUCCEED);
			return instance;
		} catch (ConstraintViolationException e) {
			LOG.error(I18N_INSERT + I18N_FAILED, e);
			throw new CorePersistenceException(CoreExceptionText.I18N_DATA_ALREADY_IN_DB, e);
		} catch (DataIntegrityViolationException e) {
			LOG.error(I18N_INSERT + I18N_FAILED, e);
			if (e.getCause() instanceof PropertyValueException) {
				throw new CorePersistenceException(CoreExceptionText.I18N_NOT_NULL_IS_NULL, e);
			} else {
				throw new CorePersistenceException(CoreExceptionText.I18N_DATA_ALREADY_IN_DB, e);
			}
		} catch (Exception re) {
			LOG.error(I18N_INSERT + I18N_FAILED, re);
			throw new CorePersistenceException(CoreExceptionText.I18N_OPERATION_FAILED, re);
		}
	}
	 ...
	 * @param coreDomain
	 */
	public final void evict(Object coreDomain) {
	 ...
	 * @return true, if successful {@inheritDoc}
	 */
	public boolean removeBulkUsingFilter(List<PropCriteriaAndValue> filter, Boolean... and) {
		LOG.debug(I18N_REMOVE_BULK + clazz.getSimpleName() + I18N_INSTANCE);
		if (filter == null || filter.isEmpty()) {
			throw new IllegalArgumentException(CoreExceptionText.I18N_FILTER_IS_EMPTY);
		}
		try {
			// construct hql
	 ...
	 */
	@Override
	public PaginationResult<T> paginateByProperty(String propertyName, Object value,
			List<String> leftJoinFetchColumns, List<QueryOrder> orders, LockMode lockMode,
			final int... rowStartIdxAndCount) {
		List<T> resultList = this.findByProperty(propertyName, value, leftJoinFetchColumns, orders,
				lockMode, rowStartIdxAndCount);
	...
	public PaginationResult<T> paginateByMapOfProperties(
			Map<String, ? extends Object> propertiesMap, List<String> leftJoinFetchColumns,
			List<QueryOrder> orders, LockMode lockMode, final int... rowStartIdxAndCount) {
		List<T> resultList = this.findByMapOfProperties(propertiesMap, leftJoinFetchColumns,
				orders, lockMode, rowStartIdxAndCount);
	...
	public PaginationResult<T> paginateUsingFilter(List<String> leftJoinFetch,
			...
			final int... rowStartIdxAndCount) {
		List<T> resultList = this.findUsingFilter(leftJoinFetch, filter, orders,
				rowStartIdxAndCount);
	...
	public PaginationResult<T> paginateLikeMapOfProperties(
		...
		List<T> resultList = this.findLikeMapOfProperties(propertiesMap, leftJoinFetchColumns,
	...
	public PaginationResult<T> paginateAll(List<String> leftJoinFetchColumns,
			List<QueryOrder> orders, final int... rowStartIdxAndCount) {
		List<T> resultList = this.findAll(leftJoinFetchColumns, orders, rowStartIdxAndCount);
	 ...
	 */
	@Override
	public PaginationResult<T> paginateLikeProperty(String propertyName, Object value,
			List<String> leftJoinFetchColumns, List<QueryOrder> orders,
			final int... rowStartIdxAndCount) {
		List<T> resultList = this.findLikeProperty(propertyName, value, leftJoinFetchColumns,
				orders, rowStartIdxAndCount);
	...
	public void remove(long id) {
		...
		Object record = getHibernateTemplate().load(clazz, id);
