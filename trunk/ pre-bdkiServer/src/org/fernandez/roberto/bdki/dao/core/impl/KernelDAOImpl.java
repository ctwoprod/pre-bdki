...
import org.fernandez.roberto.bdki.common.QueryOrder;
...
import org.fernandez.roberto.bdki.dao.core.KernelDAO;
 ...
 * 
 */
public class KernelDAOImpl<T> extends HibernateDaoSupport implements
		KernelDAO<T> {

	/** The Constant serialVersionUID. */
	private static final long serialVersionUID = -458671840345944857L;
	 ...
	 *            annotation
	 */
	public KernelDAOImpl(Class<T> clazz, SessionFactory sessionFactory) {
		super();
		this.clazz = clazz;
		super.setSessionFactory(sessionFactory);
		clazzSimpleName = clazz.getSimpleName();
		this.selectFindQuery = new StringBuilder("SELECT model FROM ").append(clazzSimpleName)
				.append(AS_MODEL).toString();
		this.selectCountQuery = new StringBuilder("SELECT COUNT(*) FROM ").append(clazzSimpleName)
				.append(AS_MODEL).toString();
		this.deleteFromQuery = new StringBuilder("DELETE FROM ").append(clazzSimpleName).append(
				AS_MODEL).toString();
		this.updateFromQuery = new StringBuilder("UPDATE ").append(clazzSimpleName)
				.append(AS_MODEL).toString();
	}
	...
	protected String getSelectFindQuery() {
		if (classNameForGenericDao != null
				&& clazzSimpleName.equals(Object.class.getSimpleName())) {
			return StringUtils.replace(selectFindQuery, clazzSimpleName, classNameForGenericDao);
		} else {
	...
	public String getSelectCountQuery() {
		if (classNameForGenericDao != null
				&& clazzSimpleName.equals(Object.class.getSimpleName())) {
			return StringUtils.replace(selectCountQuery, clazzSimpleName, classNameForGenericDao);
		} else {
	...
	public String getDeleteFromQuery() {
		if (classNameForGenericDao != null
				&& clazzSimpleName.equals(Object.class.getSimpleName())) {
			return StringUtils.replace(deleteFromQuery, clazzSimpleName, classNameForGenericDao);
		} else {
	...
	public String getUpdateFromQuery() {
		if (classNameForGenericDao != null
				&& clazzSimpleName.equals(Object.class.getSimpleName())) {
			return StringUtils.replace(updateFromQuery, clazzSimpleName, classNameForGenericDao);
		} else {
	 ...
	 * 
	 * @param factory
	 *            the new ncsis session factory
	 * @author dbejar
	 */
	public void setNcsisSessionFactory(SessionFactory factory) {
	 ...
	 * @author yosi
	 */
	protected static String createQueryOrderPhrase(List<QueryOrder> queryOrders, Boolean paginated) {
		if (paginated == null) {
			paginated = true;
		}
		...
		StringBuilder buffer = new StringBuilder(orders.size() * ORDER_BY_TEXT_ESTIMATED_LENGTH);
	 ...
	 * @return The ready to use phrase.
	 */
	private static String createJoinPhraseOfAnyKind(boolean left, String... columnNames) {
		if (columnNames == null || columnNames.length == 0) {
			return "";
		}
		StringBuilder phrase = new StringBuilder(columnNames.length * JOIN_TEXT_ESTIMATED_LENGTH);
	...
	protected static String createLeftJoinFetchPhrase(List<String> columnNames) {
		...
		return createLeftJoinFetchPhrase(columnNames.toArray(new String[columnNames.size()]));
	}
	 ...
	 * @author dbejar
	 */
	protected String retrieveSingleWhereClause(String propertyName, Object value,
	 ...
	 * @author cornelius
	 */
	protected Query createQueryForMaps(String selectQueryStr, QueryComparator comparator,
			Map<String, ? extends Object> propertiesMap, List<QueryOrder> orders,
			LockMode lockMode, Boolean... isLowercase) {
		StringBuilder queryString = new StringBuilder(selectQueryStr);
		queryString.append(retrieveSingleWhereClauseForMaps(propertiesMap, comparator));
		// row ordering
		...
		int order = 0;
		for (Entry<String, ? extends Object> propValue : propertiesMap.entrySet()) {
			Object value = propValue.getValue();
			if (value == null) {
				continue;
			}
			String propertyName = propValue.getKey();
			setsParamToValueOrToLowercaseValue(query, propertyName, comparator, value, order++,
					isLowercase);
		}
		return query;
	}
	 ...
	 * @author dbejar
	 */
	protected String retrieveSingleWhereClauseForMaps(Map<String, ? extends Object> propertiesMap,
			QueryComparator comparator) {
		...
		for (String propertyName : propertiesMap.keySet()) {
			...
			queryString.append(SqlUtil.createSingleWhereClause(propertyName, propertiesMap
					.get(propertyName), comparator));
		}
	 ...
	 */
	@SuppressWarnings(UNCHECKED)
	protected List<T> doQueryForMaps(String selectQueryStr, QueryComparator comparator,
			Map<String, ? extends Object> propertiesMap, List<QueryOrder> orders,
			LockMode lockMode, Boolean isLowercase, final int... rowStartIdxAndCount) {
		try {
			Query query = createQueryForMaps(selectQueryStr, comparator, propertiesMap, orders,
					lockMode, isLowercase);
			if (lockMode != null) {
			...
			throw new CorePersistenceException(I18N_FIND + I18N_FAILED, e);
		}
	}
	 ...
	 * @param isLowercase
	 */
	protected void setsParamToValueOrToLowercaseValue(Query query, String propertyName,
			QueryComparator comparator, Object value, Object paramPositionOrName,
			Boolean... isLowercase) {
		if (value != null) {
			...
			if (value instanceof String) {
				...
				if (SqlUtil.isLowercaseSetToYesOrNull(isLowercase) && !SqlUtil.isAnId(propertyName)) {
